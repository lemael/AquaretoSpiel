...
class LobbyScene(private val rootService: RootService) : MenuScene(1920, 1080), Refreshable{
    ...
    val sessionLabel = Label(posX = 630, posY = 200, width = 900,  text= "test",font = Font(size = 40))
    ...
    override fun refreshAfterSessionCreated(sessionId: String, playerName: String) {
        sessionLabel.text=sessionId
        textField1.text=playerName
    }

    override fun refreshAfterPlayerJoined(playerNames:List<String>) {
        for (i in playerNames.indices) {
            if (i < textFields.size) {
                textFields[i+1].text = playerNames[i]
            }
        }
    }

    override fun refreshAfterSelfJoined(hostNam: String, guests: List<String>) {
        textField1.text=hostNam
        for (i in guests.indices) {
            if (i < textFields.size) {
                textFields[i+1].text = guests[i]
            }
        }
    }
    ...
    init {
        ...
        startButton.onMouseClicked = {
            ...
            rootService.networkService.startNewHostedGame(rootService.networkService.client?.playerName!!, onlinePlayerIs)
        ...
        p2Robot.isDisabled = true
        p3Robot.isDisabled = true
        p4Robot.isDisabled = true
        p5Robot.isDisabled = true
